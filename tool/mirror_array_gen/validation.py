...
def _num(v, name, *, min_value=None, max_value=None):
    ...
    if min_value is not None and v < min_value:
        raise ValueError(f"{name} deve essere >= {min_value}")
    if max_value is not None and v > max_value:
        raise ValueError(f"{name} deve essere <= {max_value}")
    return v


def validate_config(cfg):
    if not isinstance(cfg, dict):
        raise ValueError("Il config deve essere un oggetto JSON")
    if cfg.get("schema_version") != 1:
        raise ValueError("schema_version deve essere 1")
    for section in ("light", "plane", "device", "text"):
        if not isinstance(cfg.get(section), dict):
            raise ValueError(f"Sezione obbligatoria mancante: {section}")

    light = cfg["light"]
    if light.get("mode") not in ("sun", "lamp"):
        raise ValueError("light.mode deve essere 'sun' oppure 'lamp'")
    if light["mode"] == "sun":
        sun = light.get("sun")
        if not isinstance(sun, dict):
            raise ValueError("light.sun mancante")
        if sun.get("auto"):
            for k in ("datetime_utc", "lat", "lon"):
                if k not in sun:
                    raise ValueError(f"light.sun.{k} mancante per la modalità auto")
        ...
        if not isinstance(lamp, dict) or len(lamp.get("position_m", [])) != 3:
            raise ValueError("light.lamp.position_m deve contenere 3 coordinate")
    ...
    if len(dev.get("center_m", [])) != 3:
        raise ValueError("device.center_m deve contenere 3 coordinate")
    ...
    if dev.get("mirror_shape") not in ("hex", "square"):
        raise ValueError("device.mirror_shape deve essere 'hex' o 'square'")
    ...
    if not isinstance(grid, dict) or grid.get("type") not in ("hex", "square"):
        raise ValueError("device.grid.type deve essere 'hex' o 'square'")
    if grid["type"] == "hex":
        radius = grid.get("radius")
        if not isinstance(radius, int) or radius < 0:
            raise ValueError("device.grid.radius deve essere un intero >= 0")
    else:
        for k in ("cols", "rows"):
            if not isinstance(grid.get(k), int) or grid[k] < 1:
                raise ValueError(f"device.grid.{k} deve essere un intero >= 1")

    text = cfg["text"]
    if not isinstance(text.get("string"), str) or not text["string"].strip():
        raise ValueError("text.string non può essere vuoto")
