"""Visual report: array layout and projection preview."""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def save_report(rows, ctx, cfg, path):
    ...
    plt.colorbar(sc, ax=ax1, label="pillar tilt [deg]")
    ax1.set_title(f"{cfg['device']['mirror_shape'].capitalize()} array \u2014 {len(rows)} mirrors")
    ...
    ax2.set_title(f"Projection at {cfg['plane']['distance_m']} m "
                  f"(surface tilt {cfg['plane']['tilt_deg']} deg)")
    ax2.set_xlabel("m (right)"); ax2.set_ylabel("m (up)")
    fig.tight_layout()
